...
public class SteamSocketManager : SocketManager
{
    ...
    private void CreateSteamSocketServer()
    {
        ...
        steamConnectionManager = SteamNetworkingSockets.ConnectRelay<SteamConnectionManager>(SteamManager.Instance.PlayerSteamId);
    ...
    private void JoinSteamSocketServer()
    {
        if (NOT_HOST)
        {
            ...
            steamConnectionManager = SteamNetworkingSockets.ConnectRelay<SteamConnectionManager>(SteamManager.Instance.OpponentSteamId, 0);
    ...
    public void RelaySocketMessageReceived(IntPtr message, int size, uint connectionSendingMessageId)
    {
        try
        {
            ...
            foreach (Connection c in steamSocketManager.Connected)
            {
                if (c.Id != connectionSendingMessageId)
                {
                    Result success = c.SendMessage(message, size);
                    if (success != Result.OK)
                    {
                        Result retry = c.SendMessage(message, size);
                    }
                }
            }
        }
